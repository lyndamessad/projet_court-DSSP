...
def found_helix_angl(dict_res,dico_angles,helix):
	...
	-----
	dict_res : dictionary of patern
	dico_angles of angles ,
	helix : 3-helix/4-helix/5-helix
...
def get_write(pdbH, dico, bend, chirality, H, G, I, E):
	...
	"""
	args = str(pdbH).split("H")[0] +"_parsing_results." + str(pdbH).split(".")[1]
...
def main():
	...
	found=found_helix(patterns)
	#print(found)
	#get angles phi and psi 
	dico_angles=get_angles(dict_coord)
	#print(dico_angles["Psi"])
	#patterns=found_patterns(LH)
	#print(patterns.keys())
	#print((dico_angles["Psi"].keys()))
	#print("3turns {}\n".format(patterns["3turns"]))
	#print("4turns {}\n".format(patterns["4turns"]))
	...
	# ---------------- HELIX -------------------
	#found strand
	E= found_strand_angl(dico_paterns,dico_angles)
